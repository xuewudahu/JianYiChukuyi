...
public class RqCameraManager {
     ...
     * 解码尺寸，固定数据
     */
    public  static int DecodePreviewWidth = 3840; //2688;//3840;//2560;//2720;//3264//2720;//3840;//1920;
    public  static int DecodePreviewHeight = 2160; //1512;//2160;//2304;//2448,2160;//1080;
    /**
     * 关键属性
     */
    ...
    public RqCameraManager(Context context, @NonNull String cameraId) {
        ...
        try {
            Size[] availablePreviewSizes2 = mCameraManager.getCameraCharacteristics(mCameraId).get(
                    CameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP).getOutputSizes(SurfaceTexture.class);
            for (Size previewSize2 : availablePreviewSizes2) {
                if (previewSize2.getWidth() == 3840 && previewSize2.getHeight() == 2160) {
                    DecodePreviewWidth = 3840;
                    DecodePreviewHeight= 2160;
                }
                }
        } catch (Exception e) {
            e.printStackTrace();
                        ...
                        new CameraDevice.StateCallback() {
                            ...
                            public void onOpened(CameraDevice camera) {
                                ...
                                try {
                                     ...
                                     * 配置预览回调
                                     */
                                    //textureView.getSurfaceTexture().setDefaultBufferSize(DecodePreviewWidth, DecodePreviewHeight);
                                    Log.i(TAG, "onOpened_setDefaultBufferSize");
                                    mCamera.createCaptureSession(surfaceList, new CameraCaptureSession.StateCallback() {
                                        @Override
                                        public void onConfigured(CameraCaptureSession cameraCaptureSession) {
                                            Log.i(TAG, "onOpened_onConfigured");
                                            if (mCamera == null) {
                                                if (MiscUtil.DEBUG)
...
//                                               previewRequestBuilder.set(CaptureRequest.CONTROL_AF_TRIGGER,CaptureRequest.CONTROL_AF_TRIGGER_IDLE);
                                                ...
                                                Log.i(TAG, "onOpened_previewRequestBuilder");
    ...
    public Size getTargetPreviewSize(Size surfaceSize) {
        ...
        try {
            ...
            for (Size previewSize : availablePreviewSizes) {
                ...
                if (MiscUtil.DEBUG)
                    Log.v(TAG, "newDiff = " + newDiffs + " " + surfaceSize.getWidth() + " " + surfaceSize.getHeight());
                Log.v(TAG, "newDiffs = " + previewWidth + " " + previewHeight);
                if (newDiffs == 0) {
        ...
        Log.v(TAG, "best = " + bestPreviewWidth + " " + bestPreviewHeight);
